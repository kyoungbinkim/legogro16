...
pub fn gipa_verify_tipp_mipp<E: PairingEngine, T: Transcript>(
    ...
    let z_d = proof.z_d.into_projective(); // in the end must be equal to Z = D^r
    ...
    enum Op<'a, E: PairingEngine> {
        ...
        TD(&'a E::Fqk, <E::Fr as PrimeField>::BigInt),
        UD(&'a E::Fqk, <E::Fr as PrimeField>::BigInt),
    }
    ...
    final_res.zd += VariableBaseMSM::multi_scalar_mul(&zd_b, z_s.as_slice());
    ...
    let res = iters
        .fold(GipaTUZ::<E>::default, |mut res, op: Op<E>| {
            match op {
                ...
                Op::TD(tx, d) => {
                    let tx: E::Fqk = tx.pow(d);
                    res.td.mul_assign(&tx);
                }
                Op::UD(ux, d) => {
                    let ux: E::Fqk = ux.pow(d);
                    res.ud.mul_assign(&ux);
                }
            }
            res
        })
    ...
    E: PairingEngine,
{
    pub fn merge(&mut self, other: &Self) {
        ...
        self.td.mul_assign(&other.td);
        self.ud.mul_assign(&other.ud);
