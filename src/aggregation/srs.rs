...
use ark_std::{rand::Rng, string::ToString, vec::Vec, One, UniformRand};
...
pub struct VerifierSRSProjective<E: PairingEngine> {
    pub n: usize,
...
impl<E: PairingEngine> VerifierSRS<E> {
    pub fn to_projective(&self) -> VerifierSRSProjective<E> {
        VerifierSRSProjective {
            n: self.n,
            g: self.g.into_projective(),
            h: self.h.into_projective(),
            g_alpha: self.g_alpha.into_projective(),
            g_beta: self.g_beta.into_projective(),
            h_alpha: self.h_alpha.into_projective(),
            h_beta: self.h_beta.into_projective(),
        }
    }
}
...
pub(crate) fn structured_generators_scalar_power<G: ProjectiveCurve>(
    ...
    G::batch_normalization_into_affine(&powers_of_g)
}
